...
//$app->get('/serviceConcatStatusUsersNewsLetter/:iduser','getconcat_statususernewsletterByIdUser');
/*update*/
...
//$app->post('/serviceConcatStatusUsersNewsLetter/update/:idconcat_statususernewsletter','updateconcat_statususernewsletterByIdGroup');
...
function getconcat_statususernewsletterByIdUser($iduser) // TODO
{

    $sql = "SELECT * FROM concat_statususernewsletter WHERE iduser=:iduser";
    try {
        $db = getDB();
        $stmt = $db->prepare($sql);
        ...
        $concat_statususernewsletter = $stmt->fetchAll(PDO::FETCH_OBJ);
        $db = null;
        echo '{"iduser": ' . json_encode($concat_statususernewsletter) . '}';
